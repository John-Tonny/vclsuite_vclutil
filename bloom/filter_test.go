...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := vclutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(vclutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := vclutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := vclutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
