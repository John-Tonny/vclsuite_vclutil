...
func (cs *CoinSet) TotalValue() (value vclutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue vclutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue vclutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount vclutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue vclutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue vclutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue vclutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue vclutil.Amount, coins []Coin) (Coins, error) {
...
// vclutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() vclutil.Amount {
	return vclutil.Amount(c.txOut().Value)
}
